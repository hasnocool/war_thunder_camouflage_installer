...
def generate_file_tree_with_lsd(base_dir):
    """Generate a directory and file tree using the 'lsd --tree' command with UTF-8 decoding."""
    try:
        result = subprocess.run(
            ...
            text=True,
            encoding='utf-8',  # Ensure output is decoded using UTF-8
            check=True
        )
        return result.stdout
    except subprocess.CalledProcessError as e:
        return f"Error generating tree with lsd: {e.stderr}"
...
def main():
    ...
    # Generate the file tree with lsd
    file_tree = generate_file_tree_with_lsd(base_dir)
    ...
    # Add the file tree generated by lsd to the output
    output += "\n" + "=" * 40 + "\nProject File Tree (lsd --tree)\n" + "=" * 40 + "\n"
    output += file_tree
